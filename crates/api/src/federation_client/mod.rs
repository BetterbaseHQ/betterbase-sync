...
impl FederationPeerManager {
    ...
    pub async fn restore_subscriptions(
        ...
        let spaces = tokens
            .into_iter()
            .map(|(space_id, token)| WsSubscribeSpace {
                id: space_id,
                since: 0,
                ucan: String::new(),
                token,
        ...
        P: serde::Serialize,
    {
        ...
        let (value, _) = self
            .call_method_with_chunks_and_id(peer, &request_id, method, params)
            .await?;
        Ok(value)
    }

    async fn call_method_with_chunks<P>(
        &self,
        peer: &peer::PeerConnection,
        method: &str,
        params: &P,
        ...
        P: serde::Serialize,
    {
        ...
        self.call_method_with_chunks_and_id(peer, &request_id, method, params)
            .await
    }

    async fn call_method_with_chunks_and_id<P>(
        &self,
        peer: &peer::PeerConnection,
        request_id: &str,
        method: &str,
    ...
    {
        let first = peer
            .call_raw(&self.key_id, &self.signing_key, request_id, method, params)
            .await;
        match first {
            Ok(result) => Ok(result),
...
fn should_retry(error: &FederationPeerError) -> bool {
    matches!(
        error,
        FederationPeerError::Closed | FederationPeerError::Connect(_)
    )
}
